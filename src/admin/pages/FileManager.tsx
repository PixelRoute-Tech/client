...
import { deleteFile, getFilesAndFolders } from "../services/fileManager";
...
const FileManager = () => {
  ...
  const { data: fileSystem, isFetching: filesLoading } = useQuery({
    queryKey: ["filemanagerFetch"],
    queryFn: getFilesAndFolders,
    refetchOnWindowFocus: false,
  });

  const { mutate: deleteItemFromPath, isPending: deleteLoading } = useMutation({
  ...
  const confirmDelete = () => {
    ...
    return (
      <>
        ...
        {/* File Preview Modal */}
        <FilePreviewModal
          fileSize={Number(previewFile?.size)}
          fileUrl={`${baseURL}${previewFile?.path || ""}`}
          isOpen={!!previewFile}
